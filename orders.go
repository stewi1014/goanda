...
func (c *Connection) CreateOrder(body OrderPayload) (OrderResponse, error) {
	or := OrderResponse{}
	err := c.sendAndUnmarshal("/accounts/"+c.accountID+"/orders", body, &or)
	return or, err
}
...
func (c *Connection) GetOrders(instrument string) (RetrievedOrders, error) {
	...
	ro := RetrievedOrders{}
	err := c.requestAndUnmarshal(endpoint, &ro)
	return ro, err
}

func (c *Connection) GetPendingOrders() (RetrievedOrders, error) {
	ro := RetrievedOrders{}
	err := c.requestAndUnmarshal("/accounts/"+c.accountID+"/pendingOrders", &ro)
	return ro, err
}
