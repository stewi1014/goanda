...
// AccountProperties has properties related to an account
...
// AccountInfo has the full details of a client's account
...
// AccountSummary has a summary representation of a client's account
...
// Instruments is a full specification of tradable instruments for the account
...
// AccountChanges has changes to the account's orders
...
// OrderDetails has information on the account's orders
...
func (c *Connection) Accounts() ([]AccountProperties, error) {
	...
	err := c.requestAndUnmarshal("/accounts", &ap)
	return ap, err
}
...
func (c *Connection) GetAccount(id string) (AccountInfo, error) {
	ai := AccountInfo{}
	err := c.requestAndUnmarshal("/accounts/"+id, &ai)
	return ai, err
}

// This function does something ¯\_(ツ)_/¯
